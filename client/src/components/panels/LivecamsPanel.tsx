// Design: "Ops Center Noir" — Volcano monitoring cameras from PHIVOLCS WOVODAT
// Two modes: PHIVOLCS snapshot images (auto-refresh) and YouTube 24/7 live streams
// Covers Mayon, Kanlaon, Taal (YouTube), and Pinatubo (YouTube)
...
const VOLCANO_CAMS: VolcanoCam[] = [
  // PHIVOLCS WOVODAT — Mayon Volcano Observatory
  {
    volcano: "Mayon",
  ...
  // PHIVOLCS WOVODAT — Kanlaon Volcano Observatory
  {
    volcano: "Kanlaon",
    name: "Cabagnaan",
    mode: "phivolcs",
...
function getPhivolcsImageUrl(cam: VolcanoCam): string {
  // PHIVOLCS image URL pattern:
  // https://wovodat.phivolcs.dost.gov.ph/img/monitoring/{volcanoCode}/{YYYYMMDD}/{camPath}_{HHMM}.png
  const now = new Date();
  ...
  // Round down to nearest 2 minutes for snapshot availability
...
export default function LivecamsPanel() {
  ...
  // Refresh PHIVOLCS images every 60 seconds
  useEffect(() => {
    refreshImage();
    if (cam.mode === "phivolcs") {
  ...
  // Try previous minutes if current snapshot not available
  const handleImgError = useCallback(() => {
    if (!imgError) {
      setImgError(true);
      // Try 4 minutes ago instead
      const now = new Date();
      const pht = new Date(now.getTime() + (8 * 60 + now.getTimezoneOffset()) * 60000);
  ...
  // Group cams by volcano for the selector
  ...
  return (
    <PanelWrapper title="Volcano Cams" icon="VCAM" status="live">
      <div className="flex flex-col h-full gap-1.5">
        {/* Main view */}
        <div className="flex-1 relative rounded overflow-hidden bg-[oklch(0.08_0.01_260)] min-h-0">
          {cam.mode === "youtube" ? (
            <iframe
              key={cam.videoId}
        ...
        <div className="shrink-0 space-y-1 overflow-y-auto max-h-[120px]">
          {Object.entries(volcanoGroups).map(([volcano, indices]) => (
            <div key={volcano}>
              <div className="text-[8px] font-bold font-mono text-[oklch(0.45_0.01_260)] tracking-wider uppercase mb-0.5">
                {volcano}
              </div>
                  ...
                  return (
                    <button
                      ...
                      className={`shrink-0 rounded px-2 py-1 transition-all text-left ${
                        i === selected
                          ? "bg-[oklch(0.22_0.03_260)] border border-[#CE1126] shadow-[0_0_6px_#CE112640]"
                          : "bg-[oklch(0.15_0.02_260)] border border-transparent hover:border-[oklch(0.30_0.02_260)]"
                      }`}
                    >
                      <div className="flex items-center gap-1">
                        <span className="text-[7px] font-mono font-bold px-1 py-0.5 rounded bg-[oklch(0.12_0.02_260)] text-[oklch(0.55_0.01_260)]">
                          {c.mode === "phivolcs" ? "IMG" : "LIVE"}
                        </span>
                        <span className="text-[10px] font-semibold text-[oklch(0.85_0.005_260)] whitespace-nowrap">
                          {c.name}
                        </span>
                      </div>
                    </button>
